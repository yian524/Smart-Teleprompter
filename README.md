...
### 講稿自動抽取（載入投影片時）
- **PPTX**：直接讀每頁的備忘稿（speaker notes）
- **HTML**：讀 `data-speaker-notes` 屬性或 `<aside class="notes">`（Claude Design / reveal.js 皆可）
- **PDF**：沒有 notes 概念 → 取頁面文字當草稿
- **先預覽再載入**：跳出逐頁視窗（左縮圖、右編輯框），可像 Word 一樣點選、貼上、修改，
  確認後才成為講稿；也可以按「略過」只載投影片

### 快速試玩（`檔案 > 🧪 載入範例`）
內建 6 頁範例投影片（每頁都有備忘稿）＋ 範例 Q&A 庫，一鍵載入即可體驗
「抽講稿 → 預覽編輯 → Q&A 自動翻頁」的完整流程，不必自己準備檔案。
...
- **多種問法**：`Q:` 一行用 `/` 分隔多個講法，或加 `K:` 關鍵詞行，任一命中即可
- **自動翻投影片**：答案裡寫 `【投影片 12 頁】`、`【第 7 頁】` 或 `【翻到備答 B03 …】`，命中時自動翻到該頁
- **答稿卡拉 OK**：面板上的滑動開關，開啟後念答稿會逐字淡灰標示已念部分
- **沒把握就不亂答**：相似度低於門檻時只提示「未匹配」，不會塞一段錯答案給你照唸
- **記住上次的庫**：關掉再開會自動載入最後使用的 Q&A 檔
- **Top-3 候選**：信心不足時列出多個可能答案
- **辨識語言切換**：中/英/自動，適合國際會議

QA 庫格式（`.md` / `.txt` / `.json` 皆可）：

```
Q: 為什麼選這五個維度 / why these five dimensions
K: dimension選擇, 維度依據
A: 【翻到備答 B01 五維度 頁】 五個維度涵蓋三個層次……
```
